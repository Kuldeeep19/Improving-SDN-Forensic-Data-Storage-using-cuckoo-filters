...
class SoftSwitch:
    ...
    def __init__(self, switch_id, capacity=2000, fpr=0.01,
                 use_forensic_manager=False, archive_dir='forensic_archives',
                 fingerprint_bits=None):
        self.id = switch_id
        self.bloom = BloomFilter(capacity=capacity, false_positive_rate=fpr,
                                 verbose=False)
        if use_forensic_manager:
            self.cuckoo = ForensicFilterManager(
                capacity=capacity, false_positive_rate=fpr,
                archive_dir=f"{archive_dir}/switch_{switch_id}",
                verbose=False, fingerprint_bits=fingerprint_bits)
        else:
...
class SDNNetwork:
    ...
    def __init__(self, num_switches=6, num_hosts=4,
                 ...
                 topology='linear', use_forensic_manager=False,
                 archive_dir='forensic_archives', fingerprint_bits=None):
        ...
        self.switches = {
            i: SoftSwitch(i, capacity=switch_capacity, fpr=fpr,
                          use_forensic_manager=use_forensic_manager,
    ...
    def _build_tree_graph(self):
        """Binary tree topology (core-aggregation-access layers).

        Example for 7 switches:
                    S0 (core)
                   /    \\
                 S1      S2  (aggregation)
                / \\     / \\
              S3  S4  S5  S6  (access/edge)
        """
        adj = {i: set() for i in range(self.num_switches)}
        for i in range(self.num_switches):
            left = 2 * i + 1
            right = 2 * i + 2
            if left < self.num_switches:
                adj[i].add(left)
                adj[left].add(i)
            if right < self.num_switches:
                adj[i].add(right)
                adj[right].add(i)
        return adj

    def _attach_hosts_tree(self):
        """Attach hosts to leaf switches in a tree topology."""
        # Leaves = switches with no children in the tree
        leaves = [i for i in range(self.num_switches)
                  if 2 * i + 1 >= self.num_switches]
        mapping = {}
        for h in range(self.num_hosts):
            mapping[h] = leaves[h % len(leaves)]
        return mapping
